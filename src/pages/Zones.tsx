...
const Zones = () => {
  ...
  const filtered = useMemo(() => {
    let list = zones.filter((z) => z.freeBays > 0);
    if (query.trim()) {
  ...
  return (
    <li>
      <button
        type="button"
        onClick={onClick}
        className="card-soft flex flex-col w-full text-left transition-base hover:bg-muted/30 active:scale-[0.99] overflow-hidden"
      >
