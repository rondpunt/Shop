...
export const updateNativeWidget = async (endsAt: string | null, address: string | null) => {
  try {
    ...
    // Fail silently when not running inside Capacitor (e.g., standard browser dev environment)
    console.debug("[Widget] Native widget sync bypassed or unsupported in browser:", error);
  }
};
