...
import type { NearbyMapBaseProps } from "@/components/NearbyMapOsm";

type Props = NearbyMapBaseProps & {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          onLoadFailed?.(e instanceof Error ? e : new Error("Google Maps kon niet laden"));
        }
      }
    })();
    return () => {
