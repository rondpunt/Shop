...
export const HomeSessionWidget = ({ session, className, style }: HomeSessionWidgetProps) => {
  ...
  const { endSession, updateSession } = useDataSource();
  ...
  const handleExtend = async (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    tap();
    // Extend by 15 minutes (900 seconds)
    const newEndsAt = new Date(new Date(session.ends_at).getTime() + 15 * 60 * 1000).toISOString();
    await updateSession(session.id, { ends_at: newEndsAt });
  };
