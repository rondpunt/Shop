...
const ActiveSession = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setNoteValue(s.note || "");
      setLoading(false);
      // Rebuild alarms after a reload without prompting outside a user gesture.
      try {
        ...
        if (ok) {
          await scheduleSessionAlarms({
            ...
            remindBeforeMin: prefs.remindBeforeMin,
        ...
        /* Timer blijft werken als notificaties op dit toestel niet beschikbaar zijn. */
      }
    })();
  ...
  useEffect(() => {
    if (!session || session.ended_at || !expired) return;
    void (async () => {
      try {
        await cancelSessionAlarms(session.id);
        await endIt(session.id);
        setSession((current) => current ? { ...current, ended_at: new Date().toISOString() } : current);
      } catch {
        // Keep the expired timer visible if persistence is temporarily unavailable.
      }
    })();
  }, [expired, endIt, session]);
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Store the session photo with the authenticated parking record so it remains
    // available in the user's history after reopening the app.
            ...
            <div>
              {session.photo_url ? (
                <div className="relative rounded-xl overflow-hidden ring-1 ring-white/10">
                  <img src={session.photo_url} alt="Foto van de parkeerplek" className="w-full h-auto object-cover max-h-[200px]" />
                  <button
                    onClick={() => {
                ...
                >
                  <Camera className="h-4 w-4" /> Voeg foto van parkeerplek toe
                </button>
              )}
              <input
                type="file"
