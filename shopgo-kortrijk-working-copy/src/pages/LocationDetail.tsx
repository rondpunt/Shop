...
const LocationDetail = () => {
  ...
  const confirmStart = async (remindBeforeMin: number) => {
    ...
    setStarting(true);
    try {
      const notificationsEnabled = window.localStorage.getItem("notifications") !== "false";
      const wantsReminder = remindBeforeMin > 0 && notificationsEnabled;
      ...
      } else if (remindBeforeMin > 0 && !notificationsEnabled) {
        toast.info("Timer gestart zonder alarm", {
          description: "Zet timerwaarschuwingen aan in Instellingen om een melding te krijgen.",
        });
      }
      ...
      if (granted && wantsReminder) {
        try {
          ...
          toast.warning("Timer gestart, maar alarm niet ingepland", {
            description: "Houd Shop&Go geopend en controleer je timer.",
          });
        }
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-black/60 backdrop-blur-sm" onClick={close}>
      <div
        className="animate-sheet-in pb-safe max-h-[calc(100dvh-68px)] w-full overflow-y-auto rounded-t-[32px] bg-card p-5 shadow-sheet"
      ...
      >
        ...
        <div className="mt-3 flex items-baseline gap-2">
          ...
          <span className="text-base text-muted-foreground">/ {zone.totalBays} plaatsen</span>
        </div>
        ...
        <div className="mt-4 flex items-center gap-3 rounded-2xl bg-muted/55 px-4 py-3 ring-1 ring-border/50">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary/12 text-primary">
            <MapPin className="h-5 w-5" />
          </span>
          <div className="min-w-0">
            <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-muted-foreground">Locatie</div>
            <div className="truncate text-sm font-bold text-foreground">{zone.name}</div>
            <div className="mt-0.5 text-xs text-muted-foreground">Shop&Go · Kortrijk</div>
          </div>
        </div>
