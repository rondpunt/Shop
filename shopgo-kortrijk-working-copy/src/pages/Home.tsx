...
const Home = () => {
  ...
  const selectZone = (zone: ParkoZone) => {
    ...
    setSheetState(1);
    navigate(`/locatie/${encodeURIComponent(zone.id)}`);
  };
    ...
    <div className="relative h-[100dvh] w-full overflow-hidden bg-deep">
      ...
      >
        <div className="mx-auto max-w-md space-y-1.5">
          <Link
            to="/zones"
            className="flex h-[46px] w-full items-center gap-2 rounded-2xl bg-white px-4 text-sm text-slate-500 shadow-[0_10px_24px_rgba(3,12,25,.22)] ring-1 ring-slate-200/80 transition-base hover:text-slate-900"
          ...
          <div className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-[12px] font-semibold text-slate-900 shadow-[0_8px_18px_rgba(3,12,25,.18)] ring-1 ring-slate-200/80">
      ...
      <div
        ...
        className={cn(
          "sheet-surface pb-safe absolute left-0 right-0 z-20 mx-auto flex max-w-md flex-col rounded-t-[28px] bg-card text-card-foreground shadow-sheet",
      ...
      >
        ...
        >
          <div className="sheet-grip h-1.5 w-10 rounded-full bg-muted-foreground/30" />
        </button>
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-4">
          ...
          {parkoLoading && !parko ? (
            <div className="loading-card px-4 py-4">
              <div className="flex items-center gap-2 text-sm font-semibold text-muted-foreground">
                <Loader2 className="h-4 w-4 animate-spin text-primary" /> Live parkeerdata laden…
              </div>
              <div className="loading-shimmer mt-3 h-3 w-4/5 rounded-full" />
              <div className="loading-shimmer mt-2 h-3 w-3/5 rounded-full" />
            </div>
          ) : parkoError ? (
                  ...
                  <>
                    <div className="best-map-card motion-status-in w-full rounded-3xl p-4 text-left">
                      ...
                      <button
                        ...
                        className="nav-cta mt-4 flex min-h-[46px] w-full items-center justify-center rounded-xl bg-primary px-4 text-[15px] font-bold text-primary-foreground transition-transform active:scale-[0.98]"
                      >
                        Navigeer →
                      </button>
                    </div>
