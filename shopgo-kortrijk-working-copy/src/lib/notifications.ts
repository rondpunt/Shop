...
import { prefsStore } from "./preferences";

const isNative = Capacitor.isNativePlatform();

function playAlarmTone() {
  try {
    const tone = prefsStore.get().alarmTone || "classic";
    ...
    const playBeep = (freq: number, startTime: number, dur: number) => {
      ...
      osc.type = tone === "soft" ? "sine" : tone === "urgent" ? "sawtooth" : "square";
    ...
    if (tone === "soft") {
      ...
      playBeep(600, t, 0.2);
      playBeep(600, t + 0.3, 0.2);
    } else {
      for (let i = 0; i < 5; i++) {
        playBeep(800 + (i%2)*200, t + i*0.15, 0.1);
      }
    }
  } catch(e) {
    // ignore
...
/** Checks the current state without causing a browser permission prompt. */
...
const showWebNotification = async (
  ...
  // Mobile browsers deliver notifications more consistently from their active
  // service worker than from a document-owned Notification instance.
...
export async function scheduleSessionAlarms(opts: ScheduleOpts): Promise<void> {
  ...
  for (const it of items) {
    ...
    if (delay <= 0) continue;
      ...
      try {
          void showWebNotification(it.title, {
            body: it.body,
